...
const VisualModeScreen = ({ navigation, route, currentMode, onModeChange, isDarkMode, onToggleDarkMode, onModeVisibilityChange, visibleModes }) => {
  ...
  const FOCUS_DEBOUNCE_MS = 2000; // 2 segundos de debounce

  const focusRefresh = useCallback(() => {
    const now = Date.now();
    const timeSinceLastFocus = now - lastFocusTimeRef.current;
    
    // Solo refrescar si han pasado más de 2 segundos desde el último focus
    if (timeSinceLastFocus >= FOCUS_DEBOUNCE_MS && bankId) {
      lastFocusTimeRef.current = now;
      console.log('[VisualModeScreen] Focus refresh triggered');
      fetchDataFromCache();
    } else if (timeSinceLastFocus < FOCUS_DEBOUNCE_MS) {
      console.log(`[VisualModeScreen] Focus refresh skipped - too soon (${timeSinceLastFocus}ms < ${FOCUS_DEBOUNCE_MS}ms)`);
    }
