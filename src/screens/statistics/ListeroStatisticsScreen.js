...
const ListeroStatisticsScreen = ({ navigation, onModeVisibilityChange }) => {
  return (
    <ScreenWrapper>
      <ListeroStatisticsContent
...
const ListeroStatisticsContent = ({ navigation, onModeVisibilityChange }) => {
  ...
  // Función utilitaria para extraer todas las jugadas (para listero es directo)
  const extractAllPlays = () => {
    return tableData || [];
  };
  ...
  // Hook de estadísticas para listero
  const hookResult = useListeroStatistics();
  
  const {
    tableData,
    loading,
    error,
    loadPlaysData, // Usar la función que realmente existe
    updateDateRange,
    // Intentar obtener las funciones si existen
    loadAllStats: hookLoadAllStats,
    applyFilters: hookApplyFilters,
  } = hookResult || {};
  
  // Crear funciones de compatibilidad usando las que sí existen
  const loadAllStats = hookLoadAllStats || (async () => {
    if (typeof loadPlaysData === 'function') {
      ...
      return await loadPlaysData({ startDate, endDate });
    }
  });
  
  const applyFilters = hookApplyFilters || (async (filters) => {
    if (typeof loadPlaysData === 'function') {
      ...
      switch (period) {
        ...
        default:
          ...
          end = endDate || new Date(today.getFullYear(), today.getMonth() + 1, 0);
      }
      
      return await loadPlaysData({ startDate: start, endDate: end });
    }
  });
  ...
  const groupDetailsForExport = () => {
    ...
    // Para listero, tableData contiene directamente las jugadas
    const allPlays = extractAllPlays();
      ...
      const playsRecordParams = {
        ...
        groupData: {
          ...
          jugadas: (groupData.plays || []).map(play => ({
            ...
            ganancia: Number(play.ganancia_listero || 0),
            pagado: Number(play.pagado || play.monto_a_pagar || 0),
            balance: Number(play.balance_listero || 0)
          }))
        },
      ...
      >
        {allPlays && allPlays.length > 0 && (()=>{
          ...
          allPlays.forEach(play => {
            ...
            dayData.ganancia += Number(play.ganancia_listero || 0);
            dayData.balance += Number(play.balance_listero || 0);
          });
          ...
          return (
            <View>
              ...
              {(() => {
                ...
                const totalBalanceListero = playsInPeriod.reduce((sum, play) => sum + (Number(play.balance_listero) || 0), 0);
                
                return (
                  <View style={{ flexDirection:'row', flexWrap:'wrap', justifyContent:'space-between', marginHorizontal:8, marginTop:16 }}>
                    <View style={styles.inlineKpiCard}>
                      <Text style={styles.inlineKpiTitle}>{getSantiagoHeader('Ventas')}</Text>
                      <Text style={[styles.inlineKpiValue, { color:'#27AE60' }]}>{formatSantiagoMoney(totalBruto)}</Text>
                    </View>
                    <View style={styles.inlineKpiCard}>
                      <Text style={styles.inlineKpiTitle}>{getSantiagoHeader('Comisión')}</Text>
                      <Text style={[styles.inlineKpiValue, { color:'#f39c12' }]}>{formatSantiagoMoney(totalGananciaListero)}</Text>
                    </View>
                    <View style={styles.inlineKpiCard}>
                      <Text style={styles.inlineKpiTitle}>Premios</Text>
                      <Text style={[styles.inlineKpiValue, { color:'#e74c3c' }]}>{formatMoney(totalPagado)}</Text>
                    </View>
                    <View style={styles.inlineKpiCard}>
                      <Text style={styles.inlineKpiTitle}>{getSantiagoHeader('Balance')}</Text>
                      <Text style={[styles.inlineKpiValue, { color: getSantiagoValue(totalBalanceListero)>=0? '#27AE60':'#e74c3c' }]}>{formatSantiagoMoney(totalBalanceListero)}</Text>
                    </View>
                  </View>
                );
              })()}
  ...
  const renderDetailsTab = () => (
    ...
    >
      {renderListeroPlaysTable()}
    </ScrollView>
  );

  // Tabla de jugadas para listero
  const renderListeroPlaysTable = () => {
    const allPlays = extractAllPlays();
    ...
    const map = new Map();
    const validPlays = allPlays.filter(r => r.fecha_jugada || r.created_at);
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={true} style={styles.tableContainer}>
          <View style={styles.excelTable}>
            <View style={styles.excelHeaderRow}>
              ...
              <Text style={[styles.excelHeaderCell, { width: 80 }]}>{getSantiagoHeader('Ventas')}</Text>
              <Text style={[styles.excelHeaderCell, { width: 80 }]}>{getSantiagoHeader('Comisión')}</Text>
            ...
            {groups.map((g, groupIndex) => (
              ...
              >
                ...
                <View style={[styles.excelCellContainer, { width: 80 }]}>
                  <Text style={styles.excelCell} numberOfLines={1}>{formatMoney(g.totalPagado)}</Text>
                </View>
                <View style={[styles.excelCellContainer, { width: 80 }]}>
...
export default ListeroStatisticsScreen;