  ...
  const loadFromSupabase = async (userId, startDate, endDate) => {
    try {
      ...
      while (hasMore) {
        ...
        if (error) {
          console.error('[useCollectorStatistics] Error loading from Supabase:', error);
          throw error;
        }
  ...
  const loadPlaysData = async (filters = {}) => {
    ...
    try {
      ...
      if (cachedPlays.length > 0 && !forceRefresh) {
        console.log(`[useCollectorStatistics] Loaded ${cachedPlays.length} plays from cache`);
        ...
        if (needsUpdate) {
          console.log('[useCollectorStatistics] Starting incremental update...');
      ...
      console.log('[useCollectorStatistics] Loading from Supabase...');
